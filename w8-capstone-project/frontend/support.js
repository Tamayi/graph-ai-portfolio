  ...
  function walkText(node) {
    ...
    return (vals, ctx, key) => h(
      ...
      ...parts.map((p, i) => {
        ...
        // In the design editor we keep a wrapping span so interpolations remain
        // selectable. At runtime the span is purely structural and has no CSS,
        // but an HTML <span> inside an SVG <text> element renders nothing - so
        // SVG axis/label text was invisible. Returning a bare text node renders
        // correctly in both HTML and SVG contexts.
        if (document.body?.hasAttribute("data-dc-editor-on"))
